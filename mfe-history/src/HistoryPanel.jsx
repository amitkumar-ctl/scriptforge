...
const selectHistoryStatus = (s) => s.history.status;
...
export default function HistoryPanel({ onRestore, onDelete }) {
  ...
  if (status === 'loading') {
    return (
      <div className="mx-8 space-y-3">
        {[1, 2, 3].map(i => (
          <div key={i} className="h-16 rounded-xl bg-surface border border-white/7 animate-pulse" />
        ))}
      </div>
    );
  }
  ...
  if (!history.length) {
    return (
      <div className="mx-8 flex flex-col items-center justify-center h-48 border border-white/7 rounded-xl bg-surface text-muted gap-3">
        ...
        <p className="text-xs font-mono">No scripts yet — generate your first one!</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-8 space-y-3">
      {history.map((item, i) => {
        ...
        return (
          <div key={item.id || i} className="border border-white/7 rounded-xl bg-surface overflow-hidden hover:border-white/15 transition-all duration-200">
            {/* Header row */}
            <div className="flex items-center gap-3 px-4 py-3">
              <span style={{ color: platform?.color, fontSize: 16 }}>{platform?.icon}</span>

              <div className="flex-1 min-w-0">
                <p className="text-xs text-text font-mono truncate">{item.config?.topic || item.topic || 'Untitled'}</p>
                <div className="flex items-center gap-2 mt-0.5 text-[10px] text-muted font-mono">
                  ...
                  <span>·</span><span>{item.config?.duration}</span>
                  <span>·</span><span>{item.config?.tone}</span>
                  <span>·</span><span>{wordCount} words</span>
                  {isTemp && <span className="text-accent/60">· saving…</span>}
                </div>
              </div>

              <div className="flex items-center gap-1.5 shrink-0">
                <span className="text-[10px] text-muted font-mono hidden sm:block">{formatDate(item.createdAt)}</span>
                ...
                <button
                  ...
                  className="px-2.5 py-1 text-[10px] font-mono text-accent border border-accent/30 rounded bg-accent/5 hover:bg-accent/10 transition-all cursor-pointer"
                >Restore</button>

                {!isTemp && (
                  <button
                    onClick={() => onDelete && onDelete(item.id)}
                    className="px-2 py-1 text-[10px] font-mono text-muted border border-white/7 rounded hover:text-danger hover:border-danger/40 transition-all cursor-pointer"
                  >✕</button>
                )}
              </div>
            </div>
                ...
                {item.result?.full && (
                  <div className="bg-surface2 rounded-lg p-3 mb-3">
                    ...
                    <p className="text-xs font-mono text-muted leading-relaxed line-clamp-4">
                      {item.result.full.substring(0, 400)}{item.result.full.length > 400 && '…'}
                    </p>
                  </div>
                )}
                ...
                {item.result?.hashtags?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {item.result.hashtags.slice(0, 6).map((tag, j) => (
                      <span key={j} className="text-[9px] px-2 py-0.5 rounded-full font-mono text-accent bg-accent/8 border border-accent/20">{tag}</span>
                    ))}
                  </div>
                )}
              </div>
